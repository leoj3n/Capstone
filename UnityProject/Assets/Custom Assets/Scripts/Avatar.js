...
class PlayerAvatar extends MonoBehaviour {
	...
	function Awake() {
		...
		taRenderer = this.GetComponentInChildren( TextureAtlasRenderer );
		textureAtlasCube = this.transform.Find( 'Texture Atlas Cube' );
	...
	function Update() {
		if( isControllable ) {
			...
			setVerticalMovement();
			doMovement();
			Global.enforceBounds( this.transform );
	...
	function faceRight() {
		if( facing == -1 ) { // if facing left
			facing = 1; // face right
			this.transform.localScale.x *= -1.0;
		}
	}
	...
	function checkIfNearlyGrounded() {
		...
		for( var hit : RaycastHit in hits ) {
			if (hit.transform != this.transform) isNearlyGrounded = true;
		}
	}
	...
	function resizeCharacterControllerHeight( newHeight : float ) {
		if( characterController.height != newHeight ) {
			this.transform.position.y += Mathf.Max( ((newHeight - characterController.height) / 2.0), 0.0 );
	...
	function hitOtherAvatar( hit : RaycastHit, force : float, damping : float ) {
		...
		// impact effect	 
		var effect : GameObject = this.Instantiate( impactEffect, hitPoint, Quaternion.identity );
	...
	function addExplosionForce( pos : Vector3, radius : float, force : float, damping : float ) {
		...
		var dir : Vector3 = (this.transform.position - pos).normalized;
		dir.z = 0.0;
	...
	function OnControllerColliderHit( hit : ControllerColliderHit ) {
		if( hit.gameObject.CompareTag( 'PowerModify' ) && (isAlive()) ) {
			...
			this.Destroy( hit.transform.gameObject );
			return;
		}
	...
	function onSameTeam( avatar : GameObject ) : boolean {
		...
		return Global.avatarsOnSameTeam( avatar, this.gameObject );
	}
	...
	function OutOfBounds() {
		this.transform.position = Vector3( 0.0, 4.0, Global.sharedZ );
	...
	function audioPlay( sid : int ) {
		var uid : int = (this.GetInstanceID() + sid);
	...
	function audioIsPlaying( sid : int ) : boolean {
		return GameManager.instance.audioIsPlaying( this.GetInstanceID() + sid );
	}

	// this is called when using the Reset command in the inspector
	function Reset() {
		this.gameObject.tag = 'Player';
		this.gameObject.layer = 8; // Avatar layer
